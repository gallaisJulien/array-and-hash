...
puts "Il y a #{journalists.length} dans mon array de journalists"

# Quel est le handle le plus court de cette liste ?

tab_mot = Array.new

journalists.each do |mot|
    if mot.length == 4
            tab_mot << mot
    end
end
puts "Le handle le plus court de cette liste est : #{tab_mot}"

# Combien y-a-t'il de handle contenant 5 caractères (le @ ne compte pas pour un caractère)
tab_mot_5_carracteres = Array.new

journalists.each do |mot|
    if mot.length == 6
        tab_mot_5_carracteres << mot
    end
end
puts "Il y a #{tab_mot_5_carracteres.length} handle contenant 5 caractères."
...
puts "La position dans l'array de la personne @epenser est en #{journalists.index("@epenser")} position."
